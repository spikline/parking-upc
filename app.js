...
function fechaPeruISO(fecha = new Date()) {
    const peru = new Date(fecha.getTime() - 5 * 60 * 60 * 1000);
    return peru.toISOString();
}
function fechaPeruDate(fecha = new Date()) {
    return fechaPeruISO(fecha).split('T')[0];
}
...
function generarApartadoTicket(datos) {
    ...
    document.getElementById('apartado-plana').classList.add('hidden');
    document.getElementById('apartado-abonado').classList.add('hidden');
...
function generarBoletaSalida(datos, fechaOut, monto, ms) {
    ...
    const fechaIn = new Date(datos.fecha_entrada);
    document.getElementById('apartado-ticket').classList.add('hidden');
    document.getElementById('apartado-plana').classList.add('hidden');
    document.getElementById('apartado-abonado').classList.add('hidden');
    document.getElementById('b-placa').innerText     = datos.placa;
    document.getElementById('b-fecha-in').innerText  = fechaIn.toLocaleDateString();
    document.getElementById('b-hora-in').innerText   = fechaIn.toLocaleTimeString();
...
async function cargarHistorial() {
    const hoy = new Date();
    const diaHoy    = fechaPeruDate();
    const inicioDia = `${diaHoy}T00:00:00-05:00`;
    const finDia    = `${diaHoy}T23:59:59-05:00`;
...
async function guardarPlana(estado) {
    ...
    const { data, error } = await _supabase.from('tarifas_planas').insert([{ placa, estado_pago: estado }]).select().single();
    if (!error) {
        inputPlaca.value = "";
        cargarTablaPlanas();
        actualizarOcupacion();
        generarTicketPlana(data);
        setTimeout(() => { window.print(); imprimirPlanaFinal(); }, 300);
    } else alert("❌ Error al registrar. Intenta de nuevo.");
}

function generarTicketPlana(datos) {
    const fecha = new Date(datos.fecha_entrada || new Date());
    document.getElementById('apartado-ticket').classList.add('hidden');
    document.getElementById('apartado-boleta').classList.add('hidden');
    document.getElementById('apartado-abonado').classList.add('hidden');
    document.getElementById('tp-placa').innerText = datos.placa;
    document.getElementById('tp-fecha').innerText = fecha.toLocaleDateString();
    document.getElementById('tp-hora').innerText  = fecha.toLocaleTimeString();
    document.getElementById('apartado-plana').classList.remove('hidden');
}

function imprimirPlanaFinal() {
    document.getElementById('apartado-plana').classList.add('hidden');
    cargarTablaPlanas();
}

function generarTicketAbonado(placa, fechaPago, fechaVence) {
    const fF = (d) => new Date(d).toLocaleDateString('es-PE', { day: '2-digit', month: '2-digit', year: 'numeric' });
    document.getElementById('apartado-ticket').classList.add('hidden');
    document.getElementById('apartado-boleta').classList.add('hidden');
    document.getElementById('apartado-plana').classList.add('hidden');
    document.getElementById('ab-placa').innerText       = placa;
    document.getElementById('ab-fecha-pago').innerText  = fF(fechaPago);
    document.getElementById('ab-fecha-vence').innerText = fF(fechaVence);
    document.getElementById('apartado-abonado').classList.remove('hidden');
}

function imprimirAbonadoFinal() {
    document.getElementById('apartado-abonado').classList.add('hidden');
    cargarAbonados();
}
...
async function registrarAbonado() {
    ...
    if (!error) {
        ...
        generarTicketAbonado(placa, hoy, vence);
        setTimeout(() => { window.print(); imprimirAbonadoFinal(); }, 300);
    } else alert("❌ Error al registrar. Intenta de nuevo.");
}
...
async function cargarCalendarioMes() {
    const inicioMes = `${anioActual}-${String(mesActual + 1).padStart(2,'0')}-01T00:00:00-05:00`;
    const finMes    = `${anioActual}-${String(mesActual + 1).padStart(2,'0')}-${new Date(anioActual, mesActual + 1, 0).getDate()}T23:59:59-05:00`;
    const inicioMesDate = `${anioActual}-${String(mesActual + 1).padStart(2,'0')}-01`;
    const finMesDate    = `${anioActual}-${String(mesActual + 1).padStart(2,'0')}-${String(new Date(anioActual, mesActual + 1, 0).getDate()).padStart(2,'0')}`;
...
async function cargarResumenMes() {
    ...
    const [
        ...
        _supabase.from('abonados').select('placa').gte('fecha_pago', inicioMes).lte('fecha_pago', finMes),
    ...
    // Efectivo y Yape acumulado del mes
...
async function cargarCierreCajaHoy() {
    ...
    const [
        ...
        _supabase.from('abonados').select('placa').gte('fecha_pago', inicioHoy).lte('fecha_pago', finHoy),
